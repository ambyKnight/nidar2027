...
## Run things by hand
    ...
    ~/airmouse_ws/scripts/dashboard.sh                                   # Web GCS Dashboard: http://localhost:8080
...
- **Never leave our nodes running between flights.** `scripts/sim_down.sh` is the only safe way to stop a run; it
  matches every node of the package by install path. Leaked nodes steal the CPU that Gazebo needs to render the
  LiDAR, Gazebo then drops scans silently, and a starved SLAM flies the drone into walls (2026-09-18, eight leaked
  `survivor_tagger` processes). `sim/test_explore.sh` now refuses to fly when scans are being dropped.
- The simulated drone collides like our real drone (330 mm tip to tip with guards = 23 cm square box, 1.3 kg;
  `sim/models/iris_standoffs_airmouse`, generated by `sim/make_iris_lidar.py`) but flies
